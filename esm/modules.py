...
class ConvTransformerLayer(nn.Module):
    ...
    def _init_submodules(self, add_bias_kv, use_esm1b_layer_norm):
        ...
        self.conv=nn.Conv1d(self.embed_dim, self.embed_dim, self.kmer, padding=(self.kmer - 1) // 2) #padding='same',padding=0, padding=(self.kmer - 1) // 2 
        self.deconv=nn.ConvTranspose1d(self.embed_dim, self.embed_dim, self.kmer, padding=(self.kmer - 1) // 2) #padding=(self.kmer - 1) // 2

    def forward(
        self, x, self_attn_mask=None, self_attn_padding_mask=None, need_head_weights=False
    ):
        residual = x #B*T*C
        inverse_padding_mask = ~self_attn_padding_mask #B*T
        inverse_padding_mask = inverse_padding_mask.unsqueeze(2) #B*T*1

        x = self.conv(x.permute(0, 2, 1)).permute(0, 2, 1) #B*T*C
        x = x * inverse_padding_mask.float() #B*T*C
        x = self.self_attn_layer_norm(x) #B*T*C
        ...
        x, attn = self.self_attn(
            ...
            key=x,
            value=x,
            key_padding_mask=self_attn_padding_mask,  #self_attn_padding_mask
            need_weights=True,
            need_head_weights=need_head_weights,
            attn_mask=self_attn_mask,
        )
